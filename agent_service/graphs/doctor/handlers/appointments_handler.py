...
class AppointmentsHandler:
    ...
    async def handle(self, state: AgentState) -> Any:
        ...
        trace("DOCTOR-APPT", state.session_id,
              f"action={action!r} | doctor_id={doctor_id!r}")

        status = intent.status if intent and intent.status not in (None, "pending", "all") else None
        ...
        if action in {"view_week_appointments", "weekly_schedule", "view_all_appointments"}:
            return await self.tool_caller.get_appointments(
